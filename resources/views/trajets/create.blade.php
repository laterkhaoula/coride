<x-app-layout>
 ...
 <x-text-input id="ville_depart" class="block mt-1 w-full" type="text" name="ville_depart" :value="old('ville_depart')" placeholder="Ex: rabat" required />
 <x-input-error :messages="$errors->get('ville_depart')" class="mt-2" />
 </div>

 <div>
 <x-input-label for="ville_arrivee" value="Ville d'arrivée" />
 <x-text-input id="ville_arrivee" class="block mt-1 w-full" type="text" name="ville_arrivee" :value="old('ville_arrivee')" placeholder="Ex: casablanca" required />
 ...
 <a href="{{ route('trajets.index') }}" class="text-sm font-semibold text-slate-600 hover:text-slate-900">Annuler</a>
 <button type="submit" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm rounded-lg shadow-md transition">
 Publier le trajet
 </button>
 </div>
 </form>
